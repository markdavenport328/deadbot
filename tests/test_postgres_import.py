...
def test_schema_version_is_eight_and_each_version_has_exactly_one_migration():
    ...
    band_memberships = sorted(migrations_dir.glob("008_*.sql"))[0].read_text(encoding="utf-8")
    assert "CREATE TABLE band_memberships" in band_memberships
    assert "UPDATE deadbot_schema_metadata SET schema_version = 8" in band_memberships
